// Static documentation content for DutyManager v3.
// Command data is derived from config/bot_commands.php (NO DATABASE).
// Discord application command option types are mapped to human-readable labels below.

export interface command_option {
    name: string;
    type: string;
    required: boolean;
    description: string;
...
// Discord slash-command option type reference (config/bot_commands.php "type" key):
// 1 = Sub Command, 3 = String, 4 = Integer, 5 = Boolean, 6 = User.

export const doc_sections: doc_section[] = [
    {
        id: 'introduction',
        title: 'Introduction',
...
export const doc_groups: string[] = Array.from(new Set(doc_sections.map((section) => section.group)));
