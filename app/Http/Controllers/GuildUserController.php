...
class GuildUserController extends Controller
{
    ...
    public function accept(GuildUser $guild_user)
    {
        $auth_user = auth()->user();

        if ($auth_user->cannot(PermissionEnum::ADD_GUILD_USERS)) {
            abort(403, 'app.error_no_permission');
        }

        $success = $this->service->acceptUser($guild_user, $auth_user);
    ...
    public function updateBulkRank(UpdateRankGuildUserRequest $request, GuildUserService $guildUserService): RedirectResponse
    {
        $data = $request->validated();
        $auth_user = auth()->user();
        $guild = SelectedGuildService::get();
        ...
        $response = $guildUserService->processBulkRankUpdate($guild, $auth_user, $data);
